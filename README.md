...
Use `--blacklist` with a file that lists the MD5 checksum of each mutation to ignore (one per line). Checksums are derived from the mutated file, the position of the change, and only the lines that actually changed, so they stay valid when unrelated code in the same file is edited. Checksums generated by mutago v2.10.1 or earlier are not valid for v2.10.2 and later.

To get the checksum for a mutation, run mutago with `--debug` and copy the 32-character hex string printed next to the mutation. For example, a result line looks like this:

```
ESCAPED example/example.go:25 (statement/remove) [checksum: a1b2c3d4e5f60123456789abcdef0123]
```

Copy only the value after `checksum:` into a blacklist file:
