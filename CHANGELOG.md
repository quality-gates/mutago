...
- The engine now sends all console output through its injected stdout and stderr writers. Before, some messages went straight to the terminal: baseline failures, quality-gate failures, GitHub annotations, per-test progress, `--debug` test output, custom `--exec` output, and worker errors. CLI output is unchanged.
...
- `expression/context-nil`, `composite/field-clear`, and `loop/condition` now skip mutations that would leave local variables or imports unused (#137).
...
### Fixed
- Mutant deduplication is now position-aware (#103). The dedup key hashed only the original node text and the replacement, so identical mutations anywhere in the run collapsed into one — every `0` → `1` literal in a codebase was tested exactly once, inflating MSI and hiding untested code. The key now includes the file and the edit's byte offsets, so mutants are deduplicated only when they produce byte-identical edits at the same location in the same file. **Note: `--blacklist` checksums generated by previous versions are invalidated** and must be regenerated.

### Changed
- `--dry-run` output no longer claims "identical mutations across files are deduplicated"; only byte-identical edits at the same location are.
