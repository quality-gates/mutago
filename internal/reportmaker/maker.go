...
var mutatorDescriptions = map[string]string{
	...
	"branch/case":                  "Replaces a switch case body with a noop, keeping a zero-value return when the case was terminating",
	"branch/if":                    "Removes an if-block body so the condition becomes a no-op, keeping a zero-value return when the branch was terminating",
	"branch/else":                  "Removes an else-block body, keeping a zero-value return when the branch was terminating",
